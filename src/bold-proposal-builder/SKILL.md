...
- **Gate 1 to 2:** Brief must have fields 1, 3 (with at least one goal category selected), 6, 14, 16 filled. See `references/stage-1-intake.md` for details.
- **Gate 2 to 3:** At least 3 trends + 3 case studies + 5 inspiration references must be sourced.
- **Gate 3 to 4:** Brand system must have all 9 fields filled.
- **Gate 4 to 5:** All four specialist outputs exist and reference the brand system.
...
Stage 5 requires `data/vendor-registry.json`. If missing or stale (>90 days), Claude prompts Hemi to populate from Drive via interactive extraction using the Google Drive MCP.
